...
class TestComputeGeneOverlapDataframe(unittest.TestCase):
    ...
    def test_shared_mode(self):
        ...
        result = compute_gene_overlap_dataframe(
            self.pairs_df, self.solution_cluster_matrix, mode="shared"
        )
    ...
    def test_disjoint_mode(self):
        ...
        result = compute_gene_overlap_dataframe(
            self.pairs_df, self.solution_cluster_matrix, mode="disjoint"
        )
    ...
    def test_missing_required_columns_raises(self):
        ...
        with self.assertRaises(ValueError):
            compute_gene_overlap_dataframe(
                pd.DataFrame({"a": [1]}), self.solution_cluster_matrix
            )
    ...
    def test_invalid_mode_raises(self):
        ...
        with self.assertRaises(ValueError):
            compute_gene_overlap_dataframe(
                self.pairs_df, self.solution_cluster_matrix, mode="bad"
            )
...
class TestFrequencyCutoff(unittest.TestCase):
    ...
    def test_cutoff_on_bimodal_distribution(self):
        ...
        freq_df = pd.DataFrame(
            {
                "Gene": [f"low_{i}" for i in range(10)]
                + [f"high_{i}" for i in range(10)],
    ...
    def test_cutoff_empty_dataframe_returns_zero(self):
        ...
        self.assertEqual(
            compute_frequency_cutoff(pd.DataFrame(columns=["Gene", "Frequency"])), 0
        )
    ...
    def test_plot_frequency_cutoff_returns_figure(self):
        ...
        self.assertListEqual(
            list(trace.marker.color), ["#1f77b4", "#1f77b4", "#b0b0b0"]
        )
    ...
    def test_plot_frequency_cutoff_empty_returns_empty_figure(self):
        ...
        fig = plot_frequency_cutoff(
            pd.DataFrame(columns=["Gene", "Frequency"]), cutoff=1
        )
...
class TestSummarizeGenes(unittest.TestCase):
    ...
    def test_summarize_genes_non_square_matrix_raises(self):
        ...
        with self.assertRaises(ValueError):
            summarize_genes(
                self.overlap_df, "Shared Genes", self.gene_ids, np.zeros((2, 3))
            )
    ...
    def test_summarize_genes_matrix_size_mismatch_raises(self):
        ...
        with self.assertRaises(ValueError):
            summarize_genes(
                self.overlap_df, "Shared Genes", ["G1"], self.similarity_matrix
            )
    ...
    def test_summarize_genes_empty_gene_sets_returns_empty_result(self):
        ...
        result = summarize_genes(
            empty_df, "Shared Genes", self.gene_ids, self.similarity_matrix
        )
    ...
    def test_summarize_genes_missing_gene_warns_without_stray_output(self):
        ...
        with warnings.catch_warnings(record=True) as caught:
            ...
            self.assertIn(
                "not found in similarity matrix", str(user_warnings[0].message)
            )
