...
class TestReadSolution(unittest.TestCase):
    ...
    def test_read_valid_file(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w+", delete=False, newline='') as temp_csv:
            ...
            # RUN
            genes, num_genes, Matrix = ReadInputCSV(temp_csv.name, n_jobs=5)
        ...
        with tempfile.NamedTemporaryFile(mode="w+", delete=False) as temp_csv:
            ...
            try:
                with self.assertRaises(ValueError) as context:
                    ReadInputCSV(temp_csv.name, n_jobs=2)
    ...
    def test_no_solution_id(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w+", delete=False, newline='') as temp_csv:
            ...
            # RUN.
            genes, num_genes, Matrix = ReadInputCSV(temp_csv.name, n_jobs=3, solutions_id_colum=1)
            os.remove(temp_csv.name)
