...
def ConsensusMatrix(Solutions_Matrix: cp.ndarray) -> Tuple[cp.ndarray, cp.ndarray]:
    ...
    try:
        ...
        upper_tri_indices = cp.triu_indices(num_cols, k=1)      # Possible unique pairs of genes (columns).
        pairs = cp.column_stack(upper_tri_indices)              # Format of pairs in 1D arrays of 2.
        ...
        Sum_ConnectivityMatrix = cp.sum(GlobalConnectivityMatrix, axis=0)

        # Square Matrix copy values.
        Coincidence_Matrix = cp.zeros((num_cols, num_cols), dtype=float)
        ...
        cp.fill_diagonal(Coincidence_Matrix,1)                              # Fill with ones.
