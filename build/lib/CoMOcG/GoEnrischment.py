...
from concurrent.futures import ProcessPoolExecutor                      # Process Managment Interface.
...
def process_row_process(
        row_dict: dict, 
        hashable_groups: list[list[frozenset]], 
        n: int, 
        organism: str, 
        ont: str, 
        convert_ids: bool, 
        keytype: str) -> np.ndarray:
    solution_pair = row_dict['Solution Pair']
    equivalent_pairs = row_dict['Equivalent Clusters']
    """
    process_row_process (function): Calculates a partial similarity matrix between two clustering solutions based on the semantic similarity 
    of the genes (Wang's measure) for the clusters considered equivalent.

    This function is applied to a row of the DataFrame generated by 'find_equivalent_clusters', which contains
    a pair of solutions and their respective pairs of clusters considered equivalent. For each equivalent pair,
    the semantic similarity between the shared genes (intersection) is calculated using Wang's distance.

    Parameters:
    - row_dict (dict): dictionary with the keys 'Solution Pair' (index tuple) and 'Equivalent Clusters' (list of index pairs).
    - hashable_groups (list[list[list[frozenset]]): Structure containing all groups as immutable sets for each solution.
    - n (int): Total number of solutions.
    - organism (str): Organism annotation database (e.g. 'org.Hs.eg.db' for humans).
    - ont (str): GO ontology to be used ('BP', 'MF' or 'CC').
    - convert_ids (bool): Indicates whether to convert the gene identifiers.
    - keytype (str): Type of input identifier (e.g. 'SYMBOL', 'ENTREZID').

    Returns:
    - np.ndarray: Partial similarity matrix (n x n), where only the cell corresponding to the comparison
    between the two solutions (symmetric) is updated.
    """
...
def process_row_process_unpack(args):
    "Secure calls for process a row."
...
def Solution_Wang_index_similarity_rpy2(
        ...
        keytype: str ="SYMBOL"):
    """
    Solution_Wang_index_similarity_Python (function): Create a similarity matrix between solutions using the wang
    ...
    - num_process: Process to use (default: 4)
    - organism (str): Organism database (default: "org.Hs.eg.db" for humans).
    - ont (str): Ontology for study.
    - convert_ids (bool): Activate convertions.
    - keytype (str): Type of identifier that have gene list (default: "SYMBOL").
    ...
    #build arguments for aux function calls.
...
def Solution_Wang_index_similarity_Python(
    ...
    - final_matrix: Matrix that allocates the distance matrix among solutions.
    """
    # Mapping ids of genes into a index for similary matrix.
    ...
    # Function to process every pair of options.
