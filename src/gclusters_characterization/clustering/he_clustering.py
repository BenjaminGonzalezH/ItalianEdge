...
2. compute_inconsistency_clustering – Automatically detect the number of clusters using the
                                       inconsistency coefficient of the linkage matrix.
3. he_clustering                    – High-level interface for fixed-k clustering, visualization, and export.
4. he_inconsistency_clustering      – High-level interface for automatic (inconsistency-based)
                                       clustering, two-panel visualization, and export.
5. _validate_distance_matrix – Validate structural properties of a distance matrix.
6. _validate_genes – Validate gene labels associated with the distance matrix.
7. _build_dendrogram_figure – Construct a single-panel interactive dendrogram figure.
8. _build_inconsistency_figure – Construct a two-panel figure (dendrogram + inconsistency profile).
9. _figure_to_html – Convert a Plotly figure into HTML.
10. _write_text – Write HTML output to disk.
11. _detect_inconsistency_cut – Detect the optimal cut point via the inconsistency coefficient.
"""
...
def compute_inconsistency_clustering(
    ...
    --------
    >>> import numpy as np
    >>> from he_clustering import compute_inconsistency_clustering, InconsistencyClusteringOptions
    >>>
