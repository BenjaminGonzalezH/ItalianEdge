...
class PipelineConfig:
    ...
    go_species_key: str = "goa_human"
    go_mapping_organism: str = "hsapiens"
    go_tax_id: int = 9606
    go_hierarchy_ontology: str = "BP"
    min_genes_per_term: int = 200
    summary_min_genes_per_term: int = 500

    summary_combined_threshold: float = 0.16
    summary_min_gene_frequency: int = 800
...
def apply_symbol_fixes(symbols: list[str]) -> list[str]:
    ...
    safe_list_replace(fixed, "MALAT1", "URS000001C914_9606")
    safe_list_replace(fixed, "C1orf56", "MENT")
    safe_list_replace(fixed, "DIP2C", "DIP2B")
    safe_list_replace(fixed, "DIP2C", "DIP2B")
    safe_list_replace(fixed, "C21orf91", "EURL")
    safe_list_replace(fixed, "C1orf35", "MMTAG2")
    safe_list_replace(fixed, "KCNQ1DN", "KCNQ1")
    safe_list_replace(fixed, "KCNQ1DN", "KCNQ1")
    safe_list_replace(fixed, "FSD1L", "FSD1")
    safe_list_replace(fixed, "VNN3P", "VNN2")
    safe_list_replace(fixed, "DSCR9", "DSCR8")
    safe_list_replace(fixed, "SAXO6", "SAXO5")
    safe_list_replace(fixed, "SACK1D", "FAM83D")
    safe_list_replace(fixed, "NEAT1", "URS0000204428_9606")
    safe_list_replace(fixed, "C6orf163", "SMIM8")
...
def run_summary_go_analysis(paths: PipelinePaths, cfg: PipelineConfig, state: PipelineState) -> None:
    ...
    target_group = list(solutions_disjointive[0][0])
...
def run_pipeline(input_filename: str = "archivo_prueba_3_25_133.csv") -> PipelineState:
    ...
    """
    cfg = PipelineConfig()
    paths = PipelinePaths.build(input_filename=input_filename, file_tag="File_2")
