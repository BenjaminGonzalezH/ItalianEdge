if __name__ == "__main__":
    ...
    Ac.plot_heatmap_matrix(Prop_m,  directory + "/Results/File_2/Prop_matrix.png",
                        x_label="Solution",
                        y_label="Solution",
                        title="Similitud por prescencia de genes en clusters",
                        show_flag=False)
    Ac.plot_heatmap_matrix(Dist_m,  directory + "/Results/File_2/Dist_matrix.png",
                        x_label="Solution",
                        y_label="Solution",
    ...
    ###### Transformación a EntrezID
    start_time = time.time()
    Genes_ID = GOe.convert_symbols_to_entrez(genes)
    end_time = time.time()
    print(f"Tiempo de ejecución (Transformación de Terminos a Gene_ID) : {end_time - start_time:.6f} segundos")

    ###### Estudio final.
    start_time = time.time()
    CS.parallel_analysis(SC_matrix[0:5], directory + "/Results/File_2_Global", min_Jaccard_Value=0.10, convert_ids=True)
    end_time = time.time()
